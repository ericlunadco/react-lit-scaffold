          ...
          .pipe(
            modifyCssUrls({
              modify(url) {
                return url.replace(
                  "../",
                  "https://intellibridge.github.io/conduit-assets/img/uswds/",
                );
              },
            }),
          )
...
function watchTask() {
  watch(
    ["./components/***/**/*.scss", "!./node_modules/**/*.scss"],
    compilescss,
  );
...
// Default Gulp task
export default series(compilescss, sassToJs, watchTask);

export const build = series(compilescss, sassToJs);
